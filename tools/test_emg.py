...
def main():
    ...
    paths_EMGData = sorted(glob.glob('./data/'+ModelConfig.modality+'/*.txt'))
    results = np.zeros((len(paths_EMGData), 4))
    for idx_subject in range(len(paths_EMGData)):
        for cross_val in range(4): # 4-fold
            ...
            train_loader, valid_loader = EMGDataloader.get_dataloader(paths_EMGData[idx_subject], cross_val)
